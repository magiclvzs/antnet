...
命令行解析器目前支持两种tag：
1. `match:"k"`表示只需要匹配字段名即可
2. `match:"kv"`表示需要匹配字段名和字段值
命令行解析器的注册需要一个结构体，比如上面例子中的GetGamerLevel，是用来查询玩家等级的，它定义了三个字段，第一个表示方法，比如get，set，reload，这种情况我只需要输入方法即可，而tag `match:"k"`则表示只需要匹配字段名即可，而Gamer字段没有tag，对于没有tag的字段，解析器会认为需要匹配字段名和值，比如输入gamer 1 会被认为合法，而gamer test则不合法，因为test不是int，最后的Level也有tag，表示只需要匹配level这个字段名即可。   
在匹配的时候会将字段名默认作为小写匹配。    
